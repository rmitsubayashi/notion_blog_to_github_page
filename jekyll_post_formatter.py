...
def write_to_file(details: JekyllEntryDetails, directory: str):
    file_content = '---\nlayout: post\n'
    file_content += 'title: "' + details.title + '"\n'
    # 2019-10-25 11:22:31 +0900
    date_str = details.date.strftime('%Y-%m-%d %H:%M:%S %z')
    file_content += 'date: ' + date_str + '\n'
    file_content += 'categories: ' + ' '.join(details.categories) + '\n'
    file_content += 'id: "' + details.unique_id + '"\n'
    file_content += '---\n'
    ...
    file_name_date = details.date.strftime('%Y-%m-%d')
    title_for_file_name = details.title.replace(' ', '-')
...
class JekyllEntryFactory:
    def from_file_path(self, file_path: str) -> JekyllEntryDetails:
        ...
        return JekyllEntryDetails(title, date, id, categories, content)

    # TODO the date is set to when the script runs. 
    # this technically isn't too big of a deal, just messy code with possible bugs
    def from_notion_entry_details(self, notion_entry_details: NotionEntryDetails) -> JekyllEntryDetails:
        jst = timezone(timedelta(hours=+9), 'JST')
        return JekyllEntryDetails(notion_entry_details.title, datetime.now(jst), notion_entry_details.notion_id, notion_entry_details.categories, notion_entry_details.content)
